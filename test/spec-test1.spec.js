...
const {decodeText, encodeText} = require('../src/utils.js');
    ...
    for (const [name, output] of encoded) {
        ...
        describe(name, () => {
            it('should encode Uint8Array by base name', () => {
                ...
                assert.equal(decodeText(out), output);
            });
            ...
            it('should decode string', () => {
                ...
                assert.equal(decodeText(out), input);
            });

            it('should prefix encoded Uint8Array', () => {
                const base = constants.names[name];
                const data = base.encode(encodeText(input));
                assert.equal(decodeText(multibase.multibase(name, encodeText(data))), output);
            });
